...
var DataAdapter = (function () {
    ...
    DataAdapter.getInstance = function () {
        if (!DataAdapter.instance) {
            DataAdapter.instance = new DataAdapter();
        }
        return DataAdapter.instance;
    };
    DataAdapter.prototype.transform = function (type, origin) {
        var dest = {}, keys;
        switch (type) {
            case constants_1.TYPE_XXBS:
                dest = origin;
                keys = {
                    come: 'lineResults0',
                    back: 'lineResults1'
                ...
                break;
            case constants_1.TYPE_BUS:
                var buses = origin.buses, line = origin.line, stations = origin.stations;
                if (!line || !stations) {
                }
                else {
                    console.dir(stations);
                    //获取时间
                    var travel = { travelTime: null, pRate: 0 };
                    if (_.isArray(buses)) {
                        for (var _b = 0, buses_1 = buses; _b < buses_1.length; _b++) {
                            var bus = buses_1[_b];
                            var travels = bus.travels;
                            if (_.isArray(travels)) {
                                for (var _c = 0, travels_1 = travels; _c < travels_1.length; _c++) {
                                    var _travel = travels_1[_c];
